<script lang="ts">
  ...
  function markValidReferences() {
    ...
    spans.forEach((span) => {
      ...
      if (
        text &&
        text.toLowerCase() !== lema.toLowerCase() &&
        getEntryByHeadword(text)
      ) {
...
<svelte:head>
  <title
    >{entry
      ? `${entry.headword} — Diccionario Torrevejense`
      : "Cargando... — Diccionario Torrevejense"}</title
  >
  <meta
    name="description"
    content={entry
      ? entry.text.slice(0, 160)
      : "Entrada del diccionario torrevejense"}
  />
</svelte:head>
...
{#if entry}
  <div class="max-w-4xl mx-auto px-4 py-8">
    <nav class="flex justify-between items-center mb-8">
      <a
        href="/"
        class="flex items-center text-slate-500 hover:text-torrevieja-blue-600 transition-colors"
      >
      ...
      <div class="flex space-x-2">
        <button
          ...
          class="p-2 rounded-full border border-slate-200 hover:bg-slate-50 transition-colors {isFavorite
    ...
    <article
      class="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 md:p-12 relative overflow-hidden"
    >
      <div class="absolute top-0 right-0 p-4">
        <a
          href="/abreviaturas"
          title="Ver abreviaturas"
          class="text-slate-300 hover:text-torrevieja-blue-400"
        >
          <Info class="h-6 w-6" />
        </a>
      </div>
    ...
    <div class="mt-8 grid grid-cols-2 gap-4">
      {#if prevEntry}
        <a
          href="/entrada/{prevEntry.headword}"
          class="flex flex-col p-4 bg-white border border-slate-200 rounded-xl hover:border-torrevieja-blue-300 transition-all group"
        >
          <span
            class="text-xs text-slate-400 flex items-center gap-1 group-hover:-translate-x-1 transition-transform"
          >
      ...
      {#if nextEntry}
        <a
          href="/entrada/{nextEntry.headword}"
          class="flex flex-col p-4 bg-white border border-slate-200 rounded-xl hover:border-torrevieja-blue-300 transition-all group text-right"
        >
          <span
            class="text-xs text-slate-400 flex items-center gap-1 justify-end group-hover:translate-x-1 transition-transform"
          >
...
{:else}
  <div class="max-w-4xl mx-auto px-4 py-20 text-center">
    ...
    <p class="mt-4 text-slate-600">
      Lo sentimos, no hemos podido encontrar la palabra solicitada.
    </p>
    <a
      href="/"
      class="mt-8 inline-block bg-torrevieja-blue-600 text-white px-6 py-2 rounded-lg"
      >Volver al inicio</a
    >
  </div>
{/if}
