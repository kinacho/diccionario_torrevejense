<script lang="ts">
    ...
    $: filteredEntries = entries.filter((e) =>
        e.headword.toLowerCase().includes(filterQuery.toLowerCase()),
    );
</script>
...
<div class="max-w-7xl mx-auto px-4 py-12">
    ...
    >
        <div>
            <h1
                class="text-6xl font-serif font-bold text-torrevieja-blue-800 uppercase leading-none"
            >
                Letra {letter}
            </h1>
        ...
        {#each filteredEntries as entry}
            ...
            >
                <div class="flex items-center justify-between mb-2">
                    <h2
                        class="text-xl font-bold text-slate-800 group-hover:text-torrevieja-blue-700 transition-colors"
                    >
                        {entry.headword}
                    </h2>
    ...
    {#if filteredEntries.length === 0}
        ...
        >
            <p class="text-slate-400">
                No se han encontrado palabras que coincidan con el filtro.
            </p>
        </div>
    {/if}
</div>
