...
<svelte:head>
  ...
  <meta name="description" content="Diccionario interactivo del habla torrevejense. Más de 1.300 entradas con etimologías, locuciones y ejemplos." />
</svelte:head>

<div class="bg-gradient-to-b from-torrevieja-blue-50 to-white pt-16 pb-24">
  <div class="max-w-4xl mx-auto px-4 text-center">
    <h1 class="text-5xl md:text-7xl font-serif font-bold text-torrevieja-blue-900 mb-6 animate-fade-in">
      Habla Torrevejense
    </h1>
    <p class="text-xl text-slate-600 mb-12 max-w-2xl mx-auto">
      Descubre la riqueza del léxico local: desde el lenguaje de las salinas hasta las voces del mar.
    </p>
    ...
    <div class="relative max-w-2xl mx-auto mb-16">
      ...
      {#if suggestions.length > 0}
        <div class="absolute z-50 w-full mt-2 bg-white border border-slate-200 rounded-xl shadow-2xl overflow-hidden text-left">
          {#each suggestions as suggestion}
            ...
            >
              ...
              <ArrowRight class="h-4 w-4 text-slate-300 group-hover:text-torrevieja-blue-500 transition-colors" />
            </button>
          {/each}
        </div>
      {/if}
    ...
    <div class="flex flex-wrap justify-center gap-4 text-sm text-slate-500">
      ...
      {#each ['abajote', 'abonico', 'achavo', 'milocha', 'salinero'] as word}
        <a href="/entrada/{word}" class="text-torrevieja-blue-600 hover:underline">{word}</a>
      {/each}
    </div>
  </div>
</div>
...
<div class="max-w-4xl mx-auto px-4 -mt-20 relative z-10 pb-20">
  <div class="bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-100">
    <div class="bg-torrevieja-blue-600 px-8 py-4 flex items-center justify-between">
    ...
    <div class="p-8">
      ...
      <div class="mt-8 flex justify-end">
        <a href="/entrada/{wotd.headword}" class="text-torrevieja-blue-600 font-bold flex items-center gap-1 hover:gap-2 transition-all">
