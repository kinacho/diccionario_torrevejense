...
<div class="max-w-7xl mx-auto px-4 py-12">
  <div class="text-center mb-16">
    <h1 class="text-4xl font-serif font-bold text-slate-800 mb-4">Estadísticas del Corpus</h1>
    <p class="text-slate-500 max-w-2xl mx-auto">
        Análisis visual de las más de 1.300 entradas recopiladas en este diccionario interactivo del habla de Torrevieja.
    </p>
  </div>

  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
    <div class="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-6">
        <div class="p-4 bg-torrevieja-blue-50 text-torrevieja-blue-600 rounded-2xl">
            <Database class="h-8 w-8" />
        </div>
        <div>
            <span class="block text-3xl font-bold text-slate-800">{stats.total}</span>
    ...
    <div class="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-6">
        ...
        <div>
            <span class="block text-3xl font-bold text-slate-800">{stats.etimCount}</span>
    ...
    <div class="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-6">
        ...
        <div>
            <span class="block text-3xl font-bold text-slate-800">{stats.locCount}</span>
    ...
    <div class="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-6">
        ...
        <div>
            <span class="block text-3xl font-bold text-slate-800">{stats.sinCount}</span>
            ...
            {#each Object.entries(stats.lettersCount).sort() as [letter, count]}
                <div class="flex-1 flex flex-col items-center gap-2 group">
                    <div class="relative w-full">
                        ...
                        ></div>
                        <div class="absolute -top-6 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 bg-slate-800 text-white text-[10px] px-1 rounded transition-opacity">
                            {count}
                        </div>
                    </div>
                    <span class="text-[10px] font-bold text-slate-300">{letter}</span>
                </div>
            {/each}
        </div>
    </div>
            ...
            {#each sortedFields as [field, count]}
                <div class="space-y-1">
                    ...
                    <div class="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                        <div class="bg-torrevieja-blue-500 h-full rounded-full" style="width: {(count / maxFieldCount) * 100}%"></div>
                    </div>
                </div>
            {/each}
        </div>
