...
<div class="max-w-7xl mx-auto px-4 py-8">
  <div class="flex flex-col md:flex-row gap-8">
          ...
          <div>
            ...
            <div class="space-y-2">
              <label class="flex items-center gap-2 text-sm text-slate-600">
                <input
                  type="checkbox"
                  bind:checked={filters.has_etim}
                  on:change={updateResults}
                  class="rounded text-torrevieja-blue-600"
                />
                Con etimología
              </label>
              <label class="flex items-center gap-2 text-sm text-slate-600">
    ...
    <div class="flex-1">
      <div class="mb-6 flex justify-between items-center">
        <h1 class="text-xl text-slate-600">
          ...
          <span class="ml-2 text-sm text-slate-400"
            >({results.length} entradas encontradas)</span
          >
        </h1>
      </div>
            ...
            >
              <div>
                <div class="flex items-center gap-3 mb-1">
                  <h2 class="text-lg font-bold text-torrevieja-blue-800">
                    {entry.headword}
                  </h2>
                ...
                <div class="flex flex-wrap gap-2 mt-3">
                  {#each entry.fields as field}
                    <span
                      class="px-2 py-0.5 bg-slate-100 text-[10px] font-bold text-slate-500 rounded uppercase tracking-tighter"
                    >
                      {field}
                    </span>
                  {/each}
              ...
              <ArrowRight
                class="h-5 w-5 text-slate-300 group-hover:text-torrevieja-blue-600 group-hover:translate-x-1 transition-all"
              />
            </a>
          {/each}
        </div>
      {:else}
        <div
          class="bg-white p-12 rounded-2xl border border-dashed border-slate-300 text-center"
        >
