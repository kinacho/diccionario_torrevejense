...
<div class="min-h-screen flex flex-col">
  <header class="bg-white border-b border-slate-200 sticky top-0 z-50">
      ...
      <div class="flex justify-between items-center h-16">
        <div class="flex items-center">
          <a href="/" class="flex items-center space-x-2">
            <span class="text-2xl font-serif font-bold text-torrevieja-blue-700"
              >Diccionario Torrevejense</span
            >
          </a>
        </div>
          ...
          <div class="hidden md:flex flex-1 max-w-md mx-8">
            ...
            >
              ...
              {#if suggestions.length > 0}
                <div
                  class="absolute z-50 w-full mt-10 bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden text-left"
                >
                  {#each suggestions as suggestion}
                    ...
                    >
                      ...
                      <ArrowRight
                        class="h-3 w-3 text-slate-300 group-hover:text-torrevieja-blue-500 transition-colors"
                      />
                    </button>
                  {/each}
                </div>
        ...
        <nav class="flex items-center space-x-4">
          <button
            on:click={randomWord}
            class="p-2 text-slate-500 hover:text-torrevieja-blue-600 transition-colors"
    ...
    <div class="bg-slate-50 border-t border-slate-200 py-2 overflow-x-auto">
      <div
        class="max-w-7xl mx-auto px-4 flex justify-center space-x-1 sm:space-x-2 text-sm font-medium text-slate-600"
      >
        {#each "ABCDEFGHIJLMNÑOPQRSTUVY".split("") as letter}
          <a
            href="/letra/{letter}"
            class="px-2 py-1 hover:text-torrevieja-blue-600 hover:bg-white rounded transition-colors"
          >
            {letter}
          </a>
        {/each}
  ...
  <footer class="bg-white border-t border-slate-200 py-8">
    <div class="max-w-7xl mx-auto px-4 text-center text-slate-500 text-sm">
      ...
      <div class="mt-2 space-x-4">
        <a
          href="/acerca"
          class="hover:text-torrevieja-blue-600 transition-colors">Acerca de</a
        >
        <a
          href="/abreviaturas"
          class="hover:text-torrevieja-blue-600 transition-colors"
          >Abreviaturas</a
        >
      </div>
    </div>
