<script lang="ts">
    ...
    function loadFavorites() {
        ...
        favorites = favsList
            .map((lema: string) => getEntryByHeadword(lema))
            .filter(Boolean);
    }
    ...
    function clearFavorites() {
        if (
            confirm("¿Estás seguro de que quieres borrar todos tus favoritos?")
        ) {
...
<div class="max-w-4xl mx-auto px-4 py-12">
    <div class="flex justify-between items-center mb-12">
        <div>
            <h1 class="text-4xl font-serif font-bold text-slate-800">
                Mis Favoritos
            </h1>
            <p class="text-slate-500 mt-2">
                Tus palabras guardadas para consultar rápidamente.
            </p>
        </div>
    ...
    {#if favorites.length > 0}
        <div class="grid gap-4">
            {#each favorites as entry}
                <div
                    class="bg-white p-6 rounded-2xl border border-slate-200 flex justify-between items-center shadow-sm hover:shadow-md transition-all"
                >
                    <a href="/entrada/{entry.headword}" class="flex-grow">
                        <div class="flex items-center gap-3">
                            <h2
                                class="text-xl font-bold text-torrevieja-blue-800"
                            >
                                {entry.headword}
                            </h2>
    ...
    {:else}
        <div
            class="bg-white p-20 rounded-3xl border-2 border-dashed border-slate-100 text-center"
        >
            <Heart class="h-12 w-12 text-slate-100 mx-auto mb-4" />
            <p class="text-slate-400">
                Aún no has guardado ninguna palabra favorita.
            </p>
            <a
                href="/"
                class="mt-6 inline-block text-torrevieja-blue-600 font-bold"
                >Ir al buscador</a
            >
        </div>
    {/if}
